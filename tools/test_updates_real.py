...
print('Running', K, 'updates on real training pair')
for k in range(K):
    ml_params, m_adam, v_adam, loss = train_step(ml_params, m_adam, v_adam, k, f, f_fine_c, dt, config.ML_LR_CNN, config.ML_LR_MLP, config.ML_ADVECTION_CLAMP, config.ML_ACCELERATION_CLAMP, config.ML_BOUNDARY_BUFFER, B=B_f, E=E_f)
